...
export class ScrollbarDirective implements OnInit, OnDestroy {
  ...
  scrollTo(x: number, y: number, speed?: number): void {
    if (this._ps) {
      ...
      element.scrollTo({ left: x, top: y, behavior: speed ? 'smooth' : 'auto' });
    }
  }
  ...
  scrollToElement(element: HTMLElement, offset?: number, speed?: number): void {
    if (this._ps) {
      ...
      const x = elementRect.left - containerRect.left + container.scrollLeft - (offset || 0);
      const y = elementRect.top - containerRect.top + container.scrollTop - (offset || 0);

      container.scrollTo({
        left: x,
        top: y,
        behavior: speed ? 'smooth' : 'auto',
      });
    }
  }
  ...
  scrollToBottom(speed?: number): void {
    if (this._ps) {
      const element = this._elementRef.nativeElement;
      this.scrollTo(0, element.scrollHeight, speed);
    }
  }
  ...
  scrollToRight(speed?: number): void {
    if (this._ps) {
      const element = this._elementRef.nativeElement;
      this.scrollTo(element.scrollWidth, 0, speed);
    }
  }
  ...
  private _updateGeometry(): void {
    ...
    try {
      ...
      const geometry = {
        ...
        isRtl: this._ps.isRtl,
        isBottomVisible: this._ps.reach.y === 'end',
        isRightVisible: this._ps.reach.x === 'end',
        contentWidth: this._ps.contentWidth,
        contentHeight: this._ps.contentHeight,
      };
